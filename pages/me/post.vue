<template>
	<view id="moments">
		...
		<!-- <view class="p-top">
			<image class="p-top-image" :src="user.backImage"></image>
			<view class="p-top-v">{{user.username}}</view>
		</view> -->
		
		<view class="home-pic" :style="'background: url(' + user.backImage + '); no-repeat center center; background-size:100% 100%;'">
			<view class="home-pic-base" >
				<view class="top-pic">
					<image class="header" :src="user.headImage"></image>
				</view>
		...
		<view v-else class="moments__post" v-for="(post,index) in page.list" :key="index" id="id" style="margin-top: 70upx;">
			...
			<view class="post_right">
				...
				<view style="display: flex; flex-direction: row; justify-content: space-between;">
					<view>
						...
						<view class="paragraph topic" @tap="toTopic(post.topicId)" v-if="post.topicTitle != null">#{{post.topicTitle}}</view>
						<view id="paragraph" class="paragraph">{{post.content}}</view>
					</view>
					<view @tap="del(index)" style="width: 70upx; height: 30upx; background-color: rgb(248, 248, 248); display: flex; flex-direction: row; justify-content: center; align-items: center; border-radius: 5upx;">
						<view style="width: 10upx; height: 10upx; background-color: rgb(113, 126, 175); border-radius: 100%;"></view>
						<view style="width: 10upx; height: 10upx; background-color: rgb(113, 126, 175); border-radius: 100%; margin-left: 10upx;"></view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			del(index){
				...
				var id = this.page.list[index].id;
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
				            var url = _this.getSessionUrl("tb/tbpost/del/" + id);
							//初始化数据
							uni.request({
								url: url,
								method: 'POST',
								success: res => {
									...
									if (res.data.code != 0){
										...
										_this.page.list.splice(index, 1);
									}
								},
								fail: () => {
									uni.hideLoading();
			...
			toTopic(topicId){
				uni.navigateTo({
					url: "/pages/topic/post?id=" + topicId
				})
			}
		}
	}
